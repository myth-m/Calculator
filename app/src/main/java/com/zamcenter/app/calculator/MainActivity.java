...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        View btnkf = findViewById(R.id.btnkf);//开方
        ...
        btnkf.setOnClickListener(listener);
    ...
    private void getResult(){
        ...
        if(op.equals("√")){
            if(s1.contains("-")){
                Toast ts = Toast.makeText(getBaseContext(),"被开方数必须为正数！",Toast.LENGTH_LONG);
                ts.show();
                result = 0; //则结果是0
            }
        }
        if(op.equals("/")){
            if(s2.equals(0)){
                Toast ts = Toast.makeText(getBaseContext(),"除数不能为0！",Toast.LENGTH_LONG);
                ts.show();
            }
        }

        if(!s1.equals("")&&!s2.equals("")&&!s1.contains("(")) {//如果包含小数点的运算
            //判断是否为纯符号、多符号
            boolean isNums1d = s2.matches("[0-9]+");
            boolean isNums2d = s2.matches("[0-9]+");
            if( isNums1d == false){
                Toast ts = Toast.makeText(getBaseContext(),"表达式有误！",Toast.LENGTH_LONG);
                ts.show();
            }
            else if(isNums2d == false){
                Toast ts = Toast.makeText(getBaseContext(),"暂不支持多符号运算！",Toast.LENGTH_LONG);
                ts.show();
            }
            else {
            ...
            if(!op.equals("√")) {
                ...
                Toast ts = Toast.makeText(getBaseContext(), "表达式不完整！", Toast.LENGTH_LONG);
                ts.show();
            }
